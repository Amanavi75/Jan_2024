...
const DoubleDiamond = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-black text-white py-32 px-4">
      ...
      <div className="relative w-full max-w-4xl bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl px-4 py-12 shadow-md">
        ...
        {/* Stage Labels */}
        <div className="absolute inset-0 flex justify-between items-center px-8 text-xs font-semibold">
      ...
      {/* Bottom Descriptions */}
      <div className="mt-10 w-full max-w-6xl flex justify-between text-center text-xs text-gray-400">
        {descriptions.map((desc, idx) => (
          <div key={idx} className="flex-1 px-2">{desc}</div>
        ))}
      </div>
    </div>
  );
